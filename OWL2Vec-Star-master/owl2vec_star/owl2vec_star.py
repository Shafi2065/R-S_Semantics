...
'''
def perform_ontology_embedding(config, output_dir="output"):
    ...
    model_.wv.save_word2vec_format(os.path.join(output_dir, "vectors.txt"), binary=False)
    model_.wv.save_word2vec_format(os.path.join(output_dir, "vectors.bin"), binary=True)
...
def extract_owl2vec_model(ontology_file, config_file, uri_doc, lit_doc, mix_doc):
    ...
    model_ = perform_ontology_embedding(config)
...
if __name__ == "__main__":
    ...
    @click.command()
    @click.option('--ontology_file', required=True, help='Input OWL file')
    @click.option('--output_dir', required=True, help='Directory to save embeddings')
    @click.option('--embed_dim', default=200, help='Embedding dimensions')
    @click.option('--walk_depth', default=5, help='Depth of random walks')
    @click.option('--walks_per_entity', default=10, help='Number of walks per entity')
    def main(ontology_file, output_dir, embed_dim, walk_depth, walks_per_entity):
        ...
        config['DOCUMENT']['Lit_Doc'] = 'yes'
        config['DOCUMENT']['Mix_Doc'] = 'yes'
        config['DOCUMENT']['cache_dir'] = output_dir
        config['DOCUMENT']['walk_depth'] = str(walk_depth)
        config['DOCUMENT']['walks_per_entity'] = str(walks_per_entity)
        config['MODEL']['embed_size'] = str(embed_dim)

        with open("temp_runtime.cfg", 'w') as configfile:
            config.write(configfile)

        extract_owl2vec_model(ontology_file, "temp_runtime.cfg", True, True, True)

        print(f"\n Embeddings generated and saved in: {output_dir}")
